...
import static org.clamshellcli.api.Context.KEY_CONTROLLERS;
import static org.clamshellcli.api.Context.KEY_PROMPT_COMPONENT;
import static org.clamshellcli.api.Context.KEY_SPLASH_SCREENS;
...
/**
 * This implementation of the Shell component loads all other components in the
 * system. In this implementation, the Shell loads
 * <ul>
 * <li>IOConsole</li>
 * <li>Prompt</li>
 * <li>Controllers
 * <li>
 * </lu>the IOConsole for setup. This component also handles non-interactive
 * mode where it executes command passed in as arguments and exits upon
 * completion.
 * 
...
public class CliShell implements Shell {
	...
	/**
	 * Implements initialization logic when the shell is launched. It works by
	 * loading the following components from the classpath in the following
	 * order:
	 * <ul>
	...
	/**
	 * Load components. Create default where possible if none found on
	 * classpath.
	 * 
	 * @param plug
	 */
	private void loadComponents(Context plug) {
		...
		try {
			...
			console.printf("WARNING: Unable to load specied prompt instance.%n" + "Will use a generic instance: %s%n ",
			        ex.getMessage());
			prompt = new Prompt() {
				@Override
			...
			context.putValue(KEY_PROMPT_COMPONENT, prompt); // save for later
															// use.
		}
		...
		if (screens != null && screens.size() > 0) {
			...
			for (SplashScreen sc : screens) {
				try {
					...
					console.printf("WARNING: unable to load/render SplashScreen instance %s%n%s%n", sc.getClass(),
					        ex.getMessage());
				}
			}
		}
		...
		if (controllers.size() > 0) {
			...
			for (InputController ctrl : controllers) {
				try {
					...
					console.printf("WARNING: unable to load/configure controller" + " %s [it will be disabled]%n%s%n",
	...
	private void unloadComponent(Context ctx) {
		...
		for (InputController ctrl : ctx.getControllers()) {
			try {
				...
				console.printf("WARNING: unable to unplug controller" + " %s:%n%s%n", ctrl.getClass(), ex.getMessage());
			}
		}
		...
		for (SplashScreen screen : ctx.getSplashScreens()) {
			try {
				...
				console.printf("WARNING: unable to unplug SplashScreen instance %s%n%s%n", screen.getClass(),
				        ex.getMessage());
			}
		}
		...
		try {
			...
			console.printf("WARNING: unable to unplug Prompt instance %s%n%s%n", ctx.getPrompt().getClass(),
			        ex.getMessage());
		}
	}
				...
				while (loopRunning.get()) {
					...
					if (controllersExist()) {
						for (InputController controller : controllers) {
							...
							if (enabled 
									&& controller.respondsTo() != null
							        && controller.respondsTo().matcher(line).matches()) {
								try {
						...
						if (!handled) {
							console.printf("%nCommand unhandled." + "%nNo controllers found to handle [%s].%n", line);
						}
					} else {
	...
	private void configureController(InputController controller) {
		...
		Map<String, Map<String, ? extends Object>> ctrlsMap = (Map<String, Map<String, ? extends Object>>) config
		        .getControllersMap();
		...
		if (ctrlsMap != null) {
			...
			if (map != null) {
				...
				Pattern pattern = (inputPattern != null) ? Pattern.compile(inputPattern) : Pattern.compile(".*");
